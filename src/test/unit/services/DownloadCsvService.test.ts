...
describe('DownloadCsvService', () => {
  test('returns a CSV export and filename for all courts', async () => {
    const service = new DownloadCsvService({
      getAllCourts: jest.fn().mockResolvedValue([
        {
          courtAddresses: [
            {
  ...
  test('returns the upstream status code when loading courts fails', async () => {
    const service = new DownloadCsvService({
      getAllCourts: jest.fn().mockResolvedValue(HttpStatusCode.InternalServerError),
    } as never);
