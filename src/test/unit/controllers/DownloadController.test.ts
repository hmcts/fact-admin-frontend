...
describe('DownloadController', () => {
  let downloadCsvService = new DownloadCsvService();
  let controller = new DownloadController(downloadCsvService);

  beforeEach(() => {
    downloadCsvService = new DownloadCsvService();
    controller = new DownloadController(downloadCsvService);
  });
  ...
  test('downloads the CSV when the export succeeds', async () => {
    ...
    const getDownloadCsvStub = stub(downloadCsvService, 'getDownloadCsv').resolves({
      csv: 'Name\nReading Crown Court',
  ...
  test('renders the generic error page when the export fails', async () => {
    ...
    const request = mockRequest({});
    const responseMock = mock(response);
    const getDownloadCsvStub = stub(downloadCsvService, 'getDownloadCsv').resolves(HttpStatusCode.InternalServerError);
