...
describe('professionalInformation repeatable fields', () => {
  ...
  test('adds repeatable rows, wires hints to inputs and hides the add button at the maximum', () => {
    ...
    expect(addedItem.querySelector('[data-professional-information-heading]')?.textContent).toBe('DX code 2 (optional)');
  ...
  test('removes repeatable rows and reindexes remaining controls', () => {
    ...
    expect(remainingItems[1].querySelector('[data-professional-information-heading]')?.textContent).toBe('DX code 2 (optional)');
