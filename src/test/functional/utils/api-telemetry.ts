...
export class ApiRecorder {
  ...
  public toJson(includeRawOverride?: boolean): string {
    const includeRaw = includeRawOverride !== undefined ? includeRawOverride : this.includeRaw;
...
export function shouldIncludeRawBodies(env: Record<string, string | undefined>): boolean {
...
export function shouldAttachApiLogs(env: Record<string, string | undefined>): boolean {
...
export function shouldEmitApiLogsToStdout(env: Record<string, string | undefined>, projectName: string): boolean {
...
export function parseOdhinApiLogMode(env: Record<string, string | undefined>): OdhinApiLogMode {
...
export function resolveApiLogStdoutLimit(env: Record<string, string | undefined>): number {
...
export function resolveApiAttachmentLimit(env: Record<string, string | undefined>): number {
...
export function resolveApiMaxLogs(env: Record<string, string | undefined>): number {
...
export function resolveApiMaxFieldChars(env: Record<string, string | undefined>): number {
...
export function buildApiAttachmentPayload(
  ...
  const summary = buildApiLogSummary(recorder.toArray(), options.summaryLimit);
  ...
  attempts.push({
    ...
    note:
      summary.truncated > 0
        ? `${summary.truncated} entr${summary.truncated === 1 ? 'y' : 'ies'} omitted from the attachment summary.`
        : 'Attachment contains summary data only.',
  });

  for (const attempt of attempts) {
    if (!Number.isFinite(options.limitBytes) || attempt.payload.length <= options.limitBytes) {
      return attempt;
    }
  }
...
function formatSummaryLine(entry: ApiLogEntry, index: number): string {
  ...
  const correlation = entry.correlationId ? ` cid=${truncateMiddle(entry.correlationId, 32)}` : '';

  let bodyNote = '';
  if (entry.request?.data) {
    bodyNote = ` req=${truncateValue(JSON.stringify(entry.request.data), SUMMARY_BODY_LIMIT)}`;
  } else if (entry.request?.form) {
    bodyNote = ` form=${truncateValue(JSON.stringify(entry.request.form), SUMMARY_BODY_LIMIT)}`;
  }
...
export function resolveApiStdoutMode(env: Record<string, string | undefined>): ApiStdoutMode {
...
export function resolveApiSummaryLimit(env: Record<string, string | undefined>): number {
