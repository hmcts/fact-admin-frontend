import type { ApiClient, ApiRequestOptions, ApiResponsePayload } from '@hmcts/playwright-common';

export type ParallelGetOptions = {
  batchSize?: number;
  requestOptions?: ApiRequestOptions;
  perRequestOptions?: (path: string, index: number) => ApiRequestOptions | undefined;
};
...
export async function parallelGet<T>(
  ...
  const results: ApiResponsePayload<T>[] = [];
