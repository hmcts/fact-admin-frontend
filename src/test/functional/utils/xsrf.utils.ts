...
export type XsrfHeaderBuilder = (sessionFile: string, extraHeaders?: Record<string, string>) => Record<string, string>;
...
export function resolveXsrfToken(sessionFile: string): string {
  ...
  if (!xsrfCookie?.value) {
    throw new Error(`XSRF token not found in session file: ${sessionFile}. Ensure the session is valid.`);
  }
