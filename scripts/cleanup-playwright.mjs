...
export function cleanupPlaywrightModules(baseDir = path.join(__dirname, '..')) {
  const commonRoot = path.join(baseDir, 'node_modules', '@hmcts', 'playwright-common');
  ...
  const targets = [{ name: '@playwright' }, { name: 'playwright' }, { name: 'playwright-core' }];

  for (const { name } of targets) {
    symlinkTargetIfNeeded(path.join(nestedModulesRoot, name), path.join(topLevelRoot, name));
  }
}
