...
const configuredOutput =
  process.env.PW_ODHIN_OUTPUT ?? process.env.PLAYWRIGHT_REPORT_FOLDER ?? path.join('test-results', 'odhin-report');
const htmlOutput = process.env.PLAYWRIGHT_HTML_OUTPUT ?? 'playwright-report';
const configuredTarget = process.env.PW_ODHIN_TARGET ?? process.env.PLAYWRIGHT_REPORT_TARGET ?? configuredOutput;
...
const apiRoot = path.resolve(process.env.API_TEST_ROOT ?? path.join('playwright-e2e', 'tests', 'api'));
